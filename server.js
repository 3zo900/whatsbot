...
function saveJson(p,d){ try{ fs.mkdirSync(path.dirname(p),{recursive:true}); fs.writeFileSync(p, JSON.stringify(d,null,2),'utf8'); }catch(e){ console.log(e.message); } }
...
app.get('/',(req,res)=>{
  const b=getBalance();
  res.json({status:"W-CENTER PRO v1 API Running ✅", domain:"wsbot.me", balance:b, endpoints:["/api/admin/balance"]});
});

app.get('/api/test',(req,res)=> res.json({ok:true, time:new Date().toISOString()}));

app.listen(PORT,()=> console.log(`✅ API Ready Editable`));
