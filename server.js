...
app.post('/api/admin/reset-all',(req,res)=>{
  saveJson(BAL,{totalCredit:0,used:0,remaining:0,history:[]});
  saveJson(STATS,{totalMerchants:0,messagesToday:0,activeNumbers:0,totalNumbers:8,abandonedCarts:0,connectedStaff:0,totalStaff:0,conversationsToday:0,avgRating:0});
...
app.post('/api/merchant/activate-ai',(req,res)=>{
  ...
  let keys=getJson(KEYS,{}); let stats=getStats();
  keys[merchantId]={id:merchantId, whatsappNumber, master:MASTER_OPENAI_KEY?MASTER_OPENAI_KEY.substring(0,20)+'...':'master', status:'active', createdAt:new Date().toISOString(), owner:'عبدالعزيز'};
  saveJson(KEYS,keys);
  stats.totalMerchants=Object.keys(keys).length; stats.activeNumbers=Object.keys(keys).length;
  saveJson(STATS,stats);
  let merchants=getJson(MERCHANTS,[]); if(!merchants.find(m=>m.id===merchantId)){merchants.push({id:merchantId,name:merchantId,domain:'new-store.com',chats:0}); saveJson(MERCHANTS,merchants);}
  res.json({success:true, message:'تم تفعيل AI من ماستر عبدالعزيز'});
});

app.get('/api/admin/balance',(req,res)=>{
  const b=getBalance();
  res.json({...b, openaiKeySet:!!MASTER_OPENAI_KEY, billingUrl:'https://platform.openai.com/account/billing/overview', owner:'عبدالعزيز - إدخال يدوي'});
});
app.post('/api/admin/balance',(req,res)=>{
  let b=getBalance(); b.totalCredit=parseFloat(req.body.totalCredit); b.remaining=Math.max(0,b.totalCredit-b.used); b.updatedAt=new Date().toISOString(); saveJson(BAL,b); res.json(b);
});
app.get('/api/admin/stats',(req,res)=>{
  const b=getBalance(); const m=getJson(MERCHANTS,[]);
  res.json({totalMerchants:m.length,messagesToday:0,activeNumbers:0,totalNumbers:8,remainingBalance:b.remaining,totalCredit:b.totalCredit,merchants:m,zeroed:true,masterKeyActive:!!MASTER_OPENAI_KEY,owner:'عبدالعزيز'});
});
